...
class Analyse(Autoencoder):
    def __init__(self, encodings, scaler, classifier, 
                vectorised_texts, df_count, word_to_id,
                vocab, phrase_lens):
        super().__init__(vectorised_texts.shape[1])
    ...
    def topic_extractor(self, encoding, num_topics, phrase_len_choices):

        # extracts a number of the most common topics from an encoding #
        # returns list of words #

        decoding = self.create_decoding(self.scaler, encoding)
        sorted_vocab = np.squeeze(self.vocab[np.argsort(decoding)])
        top_ngrams = []
        for phrase_len in phrase_len_choices:
          ngram_count = 0
          for ngram in sorted_vocab:
            if len(re.findall(' ', ngram)) == phrase_len - 1:
              top_ngrams.append(ngram)
              ngram_count += 1
            if ngram_count == num_topics:
              break
